<script>
  ...
  function onKeydown(e) {
    ...
    if (e.key === 'Escape') {
      selected.set(new Set()); searchQuery = ''; searchResults = null
      showNewFolder = false; showNewFile = false; showActionPanel = false
    }
    if (e.key === 'r' && !e.ctrlKey && !e.metaKey) loadFiles(get(currentPath))
  }

  function openNewFolder() {
    showNewFolder = true; showNewFile = false; showActionPanel = false
    tick().then(() => document.getElementById('new-folder-input')?.focus())
  }

  function openNewFile() {
    showNewFile = true; showNewFolder = false; showActionPanel = false
  ...
  async function triggerReindex() {
    ...
    if (res.ok) success('Indexing started')
  ...
  $: localMatches  = ($files?.files || []).filter(f => f.name.toLowerCase().includes(searchQuery.trim().toLowerCase()))
  $: displayFiles  = searchResults !== null ? searchResults : (isSearching ? localMatches : ($files?.files || []))
...
<div class="layout">
  ...
  <aside class="sidebar">
    <div class="sidebar-brand">
      <div class="brand-mark"><Icon name="folder" size={17} /></div>
      <strong>Fileship</strong>
    </div>

    <nav class="sidebar-nav">
      <button class="nav-item" class:active={true} onclick={() => navigateTo('')}>
        <Icon name="folder" size={15} /><span>Files</span>
      </button>
      ...
      {#if $sources.length > 1}
        ...
        {#each $sources as src (src.id)}
          <button class="nav-item" class:active={$activeSourceId === src.id}
            ...
            <Icon name="folder" size={14} /><span>{src.name}</span>
            {#if src.is_default}<span class="source-badge">default</span>{/if}
          </button>
        {/each}
      {/if}
      ...
      {#if $user?.is_admin}
        <a class="nav-item" href="/admin">
          <Icon name="users" size={15} /><span>Admin</span>
        </a>
      {/if}

      {#if $favorites.length > 0}
        <div class="nav-separator"><span>Favorites</span></div>
    ...
    <div class="sidebar-bottom">
      ...
      <button class="nav-item" onclick={() => showTrash = true}>
        <Icon name="trash" size={15} /><span>Trash</span>
      </button>
      <button class="nav-item" onclick={() => showThemePicker = !showThemePicker}>
        <Icon name="palette" size={15} /><span>Theme</span>
      </button>
    </div>
    ...
    <div class="sidebar-user">
      ...
      <div class="user-info">
        ...
        {#if $user?.is_admin}<span class="user-badge">Admin</span>{/if}
      </div>
  ...
  <div class="main">
    ...
    <header class="topbar">
      ...
      <div class="action-wrap">
        ...
        {#if showActionPanel}
          <div class="action-dropdown">
            <p class="dd-label">Create</p>
            <button class="dd-item" onclick={openNewFolder}>
              <span class="dd-icon folder"><Icon name="folder" size={14} /></span>
              New Folder <span class="shortcut">Ctrl+N</span>
            </button>
            <button class="dd-item" onclick={openNewFile}>
              <span class="dd-icon"><Icon name="file" size={14} /></span>
              New File
            </button>

            {#if hasSelected}
              <div class="dd-sep"></div>
              <p class="dd-label">Selection ({selCount})</p>
              <button class="dd-item" onclick={() => { downloadSelected(); showActionPanel = false }}>
                <span class="dd-icon"><Icon name="download" size={14} /></span>
                Download as ZIP
              </button>
              <button class="dd-item danger" onclick={() => { deleteSelected(); showActionPanel = false }}>
                <span class="dd-icon danger"><Icon name="trash" size={14} /></span>
                Delete selected
              </button>
            {/if}

            <div class="dd-sep"></div>
            <p class="dd-label">View</p>
            <div class="dd-view-row">
              <button class="view-btn" class:active={viewMode === 'list'}
                onclick={() => { setView('list'); showActionPanel = false }}>
                <Icon name="list" size={13} /> List
              </button>
              <button class="view-btn" class:active={viewMode === 'grid'}
                onclick={() => { setView('grid'); showActionPanel = false }}>
                <Icon name="grid" size={13} /> Grid
              </button>
            </div>

            <div class="dd-sep"></div>
            <button class="dd-item" onclick={triggerReindex} disabled={indexing}>
              <span class="dd-icon" class:spinning={indexing}><Icon name="refresh" size={14} /></span>
      ...
      <div class="search-center">
        ...
        <input bind:this={searchInput} class="search-input"
          placeholder="Search files… (Ctrl+K)"
          bind:value={searchQuery} oninput={onSearch}
      ...
      <!-- Rechts: Selection-Badge + View-Toggle -->
      <div class="topbar-right">
        {#if hasSelected}
          <span class="sel-badge">{selCount} selected</span>
          <button class="btn sm" onclick={downloadSelected}>
            <Icon name="download" size={13} /><span class="hide-sm">ZIP</span>
          </button>
          <button class="btn sm danger" onclick={deleteSelected}>
            <Icon name="trash" size={13} /><span class="hide-sm">Delete</span>
          </button>
        {/if}
        <div class="view-toggle">
          <button class="icon-btn" class:active={viewMode === 'list'} onclick={() => setView('list')} title="List">
            <Icon name="list" size={14} />
          </button>
          <button class="icon-btn" class:active={viewMode === 'grid'} onclick={() => setView('grid')} title="Grid">
    ...
    {#if showNewFolder}
      <div class="create-bar">
        ...
        <input id="new-folder-input" placeholder="Folder name" bind:value={newFolderName}
    ...
    {#if showNewFile}
      <div class="create-bar">
        ...
        <input id="new-file-input" placeholder="File name" bind:value={newFileName}
    ...
    <!-- File list -->
    <div class="file-wrap" class:grid-view={viewMode === 'grid'}
      bind:this={tableWrap} onscroll={onTableScroll}>
      {#if $loading || searchLoading}
        <Skeleton rows={14} />
      {:else if sorted.length === 0}
        <div class="empty-state">
          <div class="empty-icon"><Icon name={isSearching ? 'search' : 'folder'} size={28} /></div>
          <p class="empty-title">{isSearching ? `No results for "${searchQuery}"` : 'This folder is empty'}</p>
          {#if !isSearching}<p class="empty-sub">Drop files here or use Actions to create</p>{/if}
        </div>
      {:else}
        <table>
          <thead>
          ...
          <tbody>
            {#each sorted as file, i (file.path)}
              <FileRow {file} {dragOverPath} rowIndex={i}
...
<style>
  ...
  .sidebar-nav {
    flex: 1; padding: 0.6rem 0.6rem 0;
  ...
  .fav-item { padding-right: 0.3rem; cursor: pointer; }
  .nav-fav-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; }
  .fav-remove {
    background: none; border: none; color: var(--text3);
    cursor: pointer; display: flex; padding: 0.15rem; border-radius: 3px;
    flex-shrink: 0; opacity: 0; transition: opacity var(--transition), background var(--transition);
  }
  ...
  .user-name { font-size: 0.8rem; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .user-badge { font-size: 0.65rem; color: var(--accent); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; }
  ...
  .action-wrap { position: relative; flex-shrink: 0; }
  ...
  .action-dropdown {
    ...
    z-index: 200; padding: 0.5rem;
    animation: dd-in 0.15s cubic-bezier(.2,.8,.3,1) both;
  }
  @keyframes dd-in {
  ...
  .dd-item:hover:not(:disabled) { background: var(--row-hover); color: var(--text); }
  .dd-item:disabled { opacity: 0.5; cursor: not-allowed; }
  .dd-item.danger:hover { background: var(--danger-bg); color: var(--danger); }
  .dd-icon {
    width: 24px; height: 24px; display: grid; place-items: center;
    border-radius: 6px; background: var(--surface2); color: var(--text2); flex-shrink: 0;
  }
  .dd-icon.folder { color: var(--accent); background: var(--accent-soft); }
  .dd-icon.danger { color: var(--danger); background: var(--danger-bg); }
  .dd-icon.spinning { animation: spin 1s linear infinite; }
  .dd-sep { height: 1px; background: var(--border); margin: 0.4rem 0; }
  .dd-view-row { display: flex; gap: 0.4rem; padding: 0.2rem 0.5rem 0.3rem; }
  .view-btn {
    flex: 1; display: flex; align-items: center; justify-content: center; gap: 0.35rem;
    padding: 0.4rem; background: var(--surface2); border: 1px solid var(--border);
    color: var(--text2); font-size: 0.8rem; cursor: pointer; border-radius: var(--radius);
    transition: all var(--transition);
  }
  .view-btn.active { background: var(--accent-soft); border-color: var(--accent); color: var(--accent); font-weight: 600; }
  ...
  .search-icon {
    position: absolute; left: 0.75rem; color: var(--text3);
    display: flex; pointer-events: none; z-index: 1;
  }
  .search-input {
    width: 100%; padding: 0.5rem 2.6rem 0.5rem 2.2rem;
  ...
  .search-clear {
    ...
    background: none; border: none; color: var(--text3);
    cursor: pointer; display: flex; padding: 0.2rem; border-radius: 50%;
  }
  ...
  .location-bar {
    ...
    padding: 0.5rem 1.25rem; border-bottom: 1px solid var(--border2);
    background: var(--surface2); min-height: 36px;
  }
  .item-count { font-size: 0.75rem; color: var(--text3); white-space: nowrap; margin-left: 0.75rem; flex-shrink: 0; }
  .item-count strong { color: var(--text2); }

  /* ── Create bars ────────────────────────────────── */
  .create-bar {
    display: flex; align-items: center; gap: 0.5rem;
    padding: 0.55rem 1rem; background: var(--surface);
    border-bottom: 1px solid var(--border);
    animation: bar-in 0.15s ease both;
  }
  @keyframes bar-in { from { opacity: 0; transform: translateY(-6px); } to { opacity: 1; transform: none; } }
  .create-icon { display: flex; color: var(--text3); flex-shrink: 0; }
  .create-icon.folder { color: var(--accent); }
  .create-bar input {
    flex: 1; background: var(--input-bg); border: 1px solid var(--border);
    color: var(--text); padding: 0.42rem 0.65rem;
    border-radius: var(--radius); font-size: 0.875rem; min-width: 0;
  }
  ...
  .file-wrap {
    ...
    max-height: calc(100vh - 112px); background: var(--surface);
  }
  ...
  .empty-state {
    ...
    justify-content: center; padding: 6rem 2rem; gap: 0.75rem;
  ...
  .empty-title { font-size: 0.95rem; font-weight: 600; color: var(--text2); }
  ...
  .btn {
    ...
    background: var(--surface); border: 1px solid var(--border);
    color: var(--text); padding: 0.5rem 0.85rem;
    border-radius: var(--radius); cursor: pointer;
    font-size: 0.82rem; font-weight: 500; white-space: nowrap;
    transition: background var(--transition);
  }
  .btn:hover { background: var(--row-hover); }
  .btn.sm { padding: 0.38rem 0.6rem; font-size: 0.78rem; }
  ...
  .shortcut { margin-left: auto; font-size: 0.68rem; color: var(--text3); font-family: monospace; }
  ...
  :global(.grid-view tbody) {
    ...
    gap: 0.65rem; padding: 1rem; background: var(--bg);
  }
  :global(.grid-view tr) {
    ...
    animation: grid-in 0.2s ease both;
  }
  :global(.grid-view tr:hover) { border-color: var(--accent); box-shadow: var(--shadow); transform: translateY(-1px); }
  ...
  @keyframes grid-in { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: none; } }
